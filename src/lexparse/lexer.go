...
func LexFile(filename string) []Token {
	...
	tokens := []Token{}
	lexer := Lexer{BlockCommentLevel: 0}
...
func (lexer *Lexer) inBlockComment() bool {
	return lexer.BlockCommentLevel > 0
}
...
func (lexer *Lexer) grabTokens() []Token {
	...
	for lexer.hasMore() {
		finishedGrabbing, token := lexer.grabToken()
		if finishedGrabbing {
...
func (lexer *Lexer) grabToken() (bool, Token) {
	...
	if !lexer.inBlockComment() {
		...
		lexer.CurrentBlob = append(lexer.CurrentBlob, '\n')
...
func (lexer *Lexer) grabComment(nextTwoChars string) {
	...
	case "/*":
		...
		lexer.grabBlockComment()
	default:
		lexparsePanic(lexer.currentToken().Line, lexer.currentToken().Col, 1, "Expected comment starter")
	}
}
